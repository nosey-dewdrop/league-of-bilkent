...
public class FeedPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        header.add(UIHelper.createSubtitle("See what\u2019s happening around you."));
        ...
        JPanel chipRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 0));
        ...
        if (!recIds.isEmpty()) {
            ...
            fy.add(fyTitle);
            fy.add(Box.createVerticalStrut(8));
            for (int rid : recIds) {
            ...
            fy.add(Box.createVerticalStrut(16));
            body.add(fy);
        }
    ...
    public void refreshGrid() {
        ...
        if (events.isEmpty()) {
            ...
            for (Event ev : events) gridPanel.add(createMagazineCard(ev));
        }
        gridPanel.revalidate();
        gridPanel.repaint();
    }

    private JPanel createMagazineCard(Event ev) {
        int R = 12;
        JPanel card = new JPanel() {
            protected void paintComponent(Graphics g) {
                ...
                // Poster area — 65% of card height
                int posterH = (int)(h * 0.65);
                ...
                if (img != null) {
                    ...
                    // Pastel gradient background with event info
                    Color[] pastels = {
                        new Color(0xE8, 0xD5, 0xFF), new Color(0xFF, 0xD5, 0xE5),
                        new Color(0xD5, 0xF5, 0xD0), new Color(0xD5, 0xE8, 0xFF),
                        new Color(0xFF, 0xE8, 0xD5), new Color(0xD5, 0xFF, 0xF0),
                    };
                    Color bg = pastels[Math.abs(ev.getId()) % pastels.length];
                    Color bgDark = new Color(
                        Math.max(bg.getRed() - 25, 0),
                        Math.max(bg.getGreen() - 25, 0),
                        Math.max(bg.getBlue() - 25, 0));
                    g2.setPaint(new GradientPaint(0, 0, bg, w, posterH, bgDark));
                    g2.fillRect(0, 0, w, posterH);

                    // Decorative circles
                    g2.setColor(new Color(255, 255, 255, 30));
                    g2.fillOval(w - 40, -20, 80, 80);
                    g2.fillOval(-20, posterH - 40, 60, 60);

                    // Event title on poster
                    Color textC = new Color(
                        Math.max(bg.getRed() - 90, 20),
                        Math.max(bg.getGreen() - 90, 20),
                        Math.max(bg.getBlue() - 90, 20));
                    g2.setColor(textC);
                    g2.setFont(new Font("SansSerif", Font.BOLD, 15));
                    FontMetrics fm = g2.getFontMetrics();

                    // Word wrap title
                    String[] words = ev.getTitle().split(" ");
                    StringBuilder line = new StringBuilder();
                    int ty = posterH / 2 - 10;
                    int mx = 14;
                    for (String word : words) {
                        String test = line.length() == 0 ? word : line + " " + word;
                        if (fm.stringWidth(test) > w - mx * 2) {
                            g2.drawString(line.toString(), mx, ty);
                            ty += fm.getHeight();
                            line = new StringBuilder(word);
                        } else line = new StringBuilder(test);
                    }
                    if (line.length() > 0) g2.drawString(line.toString(), mx, ty);

                    // Location + date small
                    g2.setFont(new Font("SansSerif", Font.PLAIN, 10));
                    g2.setColor(new Color(textC.getRed(), textC.getGreen(), textC.getBlue(), 180));
                    g2.drawString("\uD83D\uDCCD " + ev.getLocation(), mx, posterH - 24);
                    g2.drawString("\uD83D\uDCC5 " + ev.getDateStr(), mx, posterH - 12);
                }
        ...
        topRow.setBorder(BorderFactory.createEmptyBorder(8, 10, 0, 10));
        ...
        if (!ev.getTags().isEmpty()) {
            ...
            tagLbl.setFont(new Font("SansSerif", Font.PLAIN, 10));
            tagLbl.setForeground(AppConstants.ACCENT);
            tagLbl.setAlignmentX(LEFT_ALIGNMENT);
            bottom.add(tagLbl);
        }

        JLabel capLbl = new JLabel(ev.getGoingCount() + "/" + ev.getCapacity() + " going");
        capLbl.setFont(new Font("SansSerif", Font.PLAIN, 10));
    ...
    private void editTagFilters() {
        String input = JOptionPane.showInputDialog(this,
            "Enter tags separated by comma:", "Edit Tag Filters", JOptionPane.PLAIN_MESSAGE);
        if (input != null) {
